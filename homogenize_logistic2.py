...
def main():
    ...
    homo = []
    for i in range(100):
        logist = logistic_map(k, x0, 1)[0]
        change = bit_switch.b2_x(logist)
        x0 = change
        homo.append(change)
    ...
    # hist3, bin_edges3 = np.histogram(homo, 10)
    # for i in range(len(hist3)):
    #     plt.scatter((bin_edges3[i]+bin_edges3[i+1])/2, hist3[i], s=10, c="k", marker='x')
    # plt.show()
